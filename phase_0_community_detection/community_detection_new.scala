...
def load_edges(date_from: String, date_to: String) : org.apache.spark.sql.DataFrame = {
    val SQL_EDGES = """
        SELECT src, dst
        FROM ( 
...
def load_vertices(base_table: String) : org.apache.spark.sql.DataFrame = {
    val vertices_df = sqlContext.read.parquet("simple_graph_vertices.parquet")
    vertices_df.createOrReplaceTempView("vertices")
    val vertices = sqlContext.sql("SELECT * FROM vertices")
    return vertices
}
...
def compute_community_features(vertices: org.apache.spark.sql.DataFrame, edges: org.apache.spark.sql.DataFrame) : org.apache.spark.sql.DataFrame = {
    val g_edges = edges.map{ case Row(src: Long, dst: Long) => Edge(src, dst, 0)}.rdd
    val g_vertices: RDD[(VertexId, Int)] = vertices.map{case Row(x: Long) => (x, 0)}.rdd   

    val default_vertex = (0)
  
    val graph = Graph(g_vertices, g_edges, default_vertex)
...
def run(date_from: String, date_to: String, base_table: String, output_file: String) : Unit = {
    val edges = load_edges(date_from, date_to)
    val vertices = load_vertices(base_table) 
    val community_features = compute_community_features(vertices, edges)
    community_features.write.parquet(output_file)
}


// run("20160201", "20160301", "base_20160301", "lpa_result_20160201_20160301_min_cnt_10")
